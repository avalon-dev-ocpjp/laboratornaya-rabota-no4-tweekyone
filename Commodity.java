...
public interface Commodity {
    ...
    static CommodityBuilder builder() {
         ...
         * на уровне пакета.
         */
       return new CommodityR.CommodityBuilderR();
    }
    ...
    static Collection<Commodity> random(int limit) throws IOException {
        try (RandomFileReader reader = RandomFileReader.fromSystemResource("resources/household.csv")) {
             ...
             */
           Collection <Commodity> randomCollection=new ArrayList<>();
           List<String> randomString=Stream.generate(reader::readLine)
                   .limit(limit)
                   .collect(Collectors.toList());
           for(String s:randomString){
               
               randomCollection.add(Commodity.valueOf(s));
           } 
    ...
    static Commodity valueOf(String string) {
         ...
         */
        CommodityBuilder cb=builder();
        String[] arrstr=string.split(";");
        cb.code(arrstr[0])
                .vendorCode(arrstr[1])
                .name(arrstr[2])
                .residue(Integer.parseInt(arrstr[3]))
                .price(Double.parseDouble(arrstr[4]));
